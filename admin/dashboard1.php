...
$query= "SELECT `login`.`user_id`,`login`.`email`,`is_active`,`create_date`,`expiry_time`,`expiry_date`, `login`.`password`, GROUP_CONCAT(name SEPARATOR ',') as `services` from `login`,`service`,`login_service_table` WHERE `login`.`user_id` = `login_service_table`.`login_fk` AND `login_service_table`.`service_fk` = `service`.`id` GROUP By `user_id`;";
...
<html lang="en" dir="ltr">
  ...
  <body>
    ...
    <div class="container">
      ...
      <hr />
      <div class="card">
        <h5 class="card-header">User Listing</h5>
        <div class="card-body">
          <div class="row">
              ...
              <table class='table table-bordered table-hover table-sm table-striped'>
                ...
                <tbody>
                  ...
                  <tr>
                    ...
                    <td>
                      <div class="service-list"><?php echo str_replace(',', " / ",$row['services'] ) ?> </div>
                    </td>
                    ...
                    <td><button class='btn btn-info btn-sm  bbb' onclick="updateUser('<?php echo $row['user_id'] ?>','<?php echo $row['email'] ?>','<?php echo $row['password'] ?>','<?php echo $row["is_active"]?>','<?php echo $row['expiry_time'] ?>','<?php echo $row['expiry_date'] ?>')">Edit</button>
     ...
     ?>
            ...
            <div class="modal-body">
              <form class="form" action="./edit.php" method="post">
                ...
                <div class="form-group text-left">
                  <label>Status: </label>
                  <br />
                  <input class="" type="checkbox" name="is_active" value="1"> <span id='updateCheckboxLabel'></span>
                </div>
                <div class="form-group">
                  <label>Expiry Time (Minutes)</label>
                  <input class="form-control form-control-sm" type="text" name="expiry_time" value="" required>
                ...
                <div class="form-group">
                  <label>Services</label>
                  <select class="form-control form-control-sm" multiple required style="min-height: 105px;">
                    <option>-</option>
                    <option>-</option>
                    <option>-</option>
                  </select>
      ...
      <script type="text/javascript">
        const updateUser = (user_id, email, password, is_active, expiry_time, expiry_date) => {
          ...
          updateInput.eq(4).prop("checked", is_active == 1 ? true : false)
          updateInput.eq(5).val(expiry_time)
          updateInput.eq(6).val(expiry_date)
